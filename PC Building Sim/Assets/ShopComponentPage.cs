    ...
    public void FillGpuShop()
    {
        ...
        if (allGpuComponents.Count > 0)
        {
            for (int i = 0 ; i < allGpuComponents.Count; i++)
            {
                ...
                shopItem.itemSpec1.text = gpu.manufacturer.ToString();
                shopItem.itemSpec2.text = gpu.memory.size.ToString() + " GB " + gpu.memory.type.ToString();
                shopItem.itemSpec3.text = gpu.pci_eSlot.ToString();
                shopItem.itemSpec4.text = gpu.memory.bandwidth.ToString() + " bit";
                shopItem.itemSpec5.text = gpu.coreClock.ToString() + " MHz core clock";
                shopItem.itemSpec6.text = gpu.shaderCount.ToString() + " shader cores";
        ...
        if(allRamComponents.Count > 0)
        {
            for(int i = 0; i <  allRamComponents.Count; i++)
            {
                ...
                if (CheckFitsPc(ram.memoryType , 3))
                {
                    ...
                    shopItem.itemSpec1.text = ram.manufacturer.ToString();
                    shopItem.itemSpec2.text = ram.memorySize.ToString() + " GB " + ram.memoryType.ToString();
                    shopItem.itemSpec3.text = ram.frequency.ToString() + " MHz";
                    shopItem.itemSpec4.text = "CL " + ram.latency.ToString();
                    shopItem.itemSpec5.text = ram.voltage.ToString() + "V" + " Voltage";
                    shopItem.itemId.text = i.ToString();
        ...
        if (allCpuComponents.Count > 0)
        {
            ...
            for(int i = 0; i < allCpuComponents.Count; i++)
            {
                ...
                if(CheckFitsPc(cpu.socket , 2))
                {
                    ...
                    shopItem.itemSpec1.text = cpu.manufacturer.ToString();
                    shopItem.itemSpec2.text = cpu.cores.ToString()+ " / " + cpu.threads.ToString() + " cores/threads";
                    ...
                    shopItem.itemSpec6.text = "Socket: " + cpu.socket;
                    shopItem.itemId.text = i.ToString();
        ...
        if (allMotherboardComponents.Count > 0)
        {
            for (int i = 0 ; i < allMotherboardComponents.Count; i++ )
            {
                ...
                if(CheckFitsPc(motherboard.cpuSocket , 0) && CheckFitsPc(motherboard.memoryType , 1))
                {
                    ...
                    shopItem.itemSpec1.text = motherboard.manufacturer;
                    ...
                    shopItem.itemSpec4.text = "Form Factor: " + motherboard.formFactor;
